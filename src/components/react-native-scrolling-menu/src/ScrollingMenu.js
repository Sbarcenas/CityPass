...
export default class ScrollingMenu extends PureComponent {
  ...
  scrollTo(itemNum) {
    ...
    const offset =
      layouts[itemNum].x + layouts[itemNum].width / 2 - containerWidth / 2;
    this.scrollView.scrollTo({
      x: Math.max(0, Math.min(offset, contentWidth - containerWidth)),
    });
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (selected === null) {
      if (defaultIndex in layouts && containerWidth && contentWidth) {
        this.select(defaultIndex, this.props.items[defaultIndex]);
      }
  ...
  renderItems() {
    ...
    const {items, itemStyle, selectedItemStyle, bottomStyle} = this.props;
    return items.map((item, i) => (
      <TouchableOpacity
        ...
        onLayout={({ nativeEvent: { layout: itemLayout } }) => {
          this.setState({
            layouts: Object.assign(layouts.slice(0), { [i]: itemLayout })
          });
        }}
      >
        ...
        {selected === i && <View style={bottomStyle} />}
      </TouchableOpacity>
    ));
  }
